...
def news_get_view(request):
    print("DEBUG: Entering news_get_view")  # Debugging log
    ...
    category = request.GET.get('category', '').strip()  # Corrected from department
    ...
    if category and category != "":  # Apply category filter
        query['category'] = {'$regex': f'^{category}$', '$options': 'i'}  # Ensures case-insensitive match
